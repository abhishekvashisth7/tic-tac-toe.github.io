...
window.onload = () => {//once window loaded
    ...
    selectOBtn.onclick = ()=>{
        ...
        players.setAttribute("class","players active player");  //adding third class name in active player
    }
}
    ...
    if(runBot){
        ...
        for (let i = 0; i < allBox.length; i++) { 
            if(allBox[i].childElementCount == 0){ //if span has no any child element
                array.push(i); //inserting unclicked or unselected boxes inside array means that span has no childern
            }
        }
        let randomBox = array[Math.floor(Math.random()*array.length)]; //getting random index from array
        if(array.length > 0){
            if(players.classList.contains("player")){ //if players element contains player
